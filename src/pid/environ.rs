...
impl std::str::FromStr for Environ {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        for entry in s.trim_end_matches('\0').split('\0'){
            ...
            let key: &str = kv.next().ok_or(crate::ProcErr::from("key not found"))?;
            let value: &str = kv.next().ok_or(crate::ProcErr::from("value not found"))?;
...
mod test {
    ...
    fn test_parse() {
        ...
        let correct = Environ(vec![("VERBOSE_LOGGING".to_string(), "true".to_string()), ("_".to_string(), "/bin/cat".to_string())]);
